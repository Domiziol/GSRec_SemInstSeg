...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, visible_mask=None, retain_grad=False, learn_SDF=True, class_subset = None):  # visible_mask - all anchors which radius >0 (projected)
    ...
    if is_training and class_subset is not None:
        ...
        anchorProbs = pc.get_sem_probs()
        K = anchorProbs.shape[1]
        
        if visible_mask is not None:
            anchorsVisibleInMask = torch.nonzero(visible_mask, as_tuple=False).squeeze(1)
        else:
            anchorsVisibleInMask = torch.arange(pc.get_anchor.shape[0], device=xyz.device)

        anchorsVisibleInMaskCounter = anchorsVisibleInMask.numel()
        
        # Detach other parameters so they won't be influenced by gradient from semantics
        xyzDetached = xyz.detach()
        screenspace_pointsDetached = screenspace_points.detach()
        normalDetached = normal.detach()
        opacityDetached = opacity.detach()
        scalingDetached = scaling.detach()
        rotDetached = rot.detach()
        ...
        assert gaussesMasked.numel() == anchorsVisibleInMaskCounter * pc.n_offsets, \
        "sth wrong with masking/flattening"

        assert anchorsFromActiveGausses.shape[0] == xyz.shape[0], \
        f"anchorsFromActiveGausses {anchorsFromActiveGausses.shape} must match rendered gaussians {xyz.shape[0]}"

        gaussProbs = anchorProbs[anchorsFromActiveGausses]
        assert gaussProbs.shape == (xyz.shape[0], K)

       
        def RenderProbabilities(singleClassProbs):
            # uses probabilities  as a 'color' input to rasterizer 
            colors_precomp = singleClassProbs.expand(-1, 3).contiguous()
            
            outProb, _, _, _, _ = rasterizer(
                means3D = xyzDetached,
                means2D = screenspace_pointsDetached,
                shs = None,
                colors_precomp = colors_precomp,  
                normal_precomp = pc.normal_activation(normalDetached),
                opacities = opacityDetached,
                scales = scalingDetached,
                rotations = rotDetached,
                cov3D_precomp = None
            )
            
            if outProb.dim() == 3:
                return outProb[0:1, :, :].unsqueeze(0)
            else:   # in case
                return outProb[:, :1, :, :] 

        
        H, W = rendered_image.shape[-2], rendered_image.shape[-1]
        semProbs = rendered_image.new_zeros((1, K, H, W))
        for c in class_subset:
            x = gaussProbs[:, c:c+1]
            semProbs[:, c:c+1, :, :] = RenderProbabilities(x)

        # normalize
        ones = torch.ones((gaussProbs.shape[0], 1), device=gaussProbs.device, dtype=gaussProbs.dtype)
        S = RenderProbabilities(ones)
        semProbs = semProbs / S.clamp_min(1e-6)  
    # === END OF SEMANTIC RENDERING ===
    ...
    if is_training:
        return {"render": rendered_image,
                ...
                "semantics": semProbs   # (thesis) return probabilities
                }
    else:
        return {"render": rendered_image,
                "render_depth": rendered_depth,
