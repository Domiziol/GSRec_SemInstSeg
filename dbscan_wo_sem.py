...
def setup_gaussian_scene_and_model(dataset : ModelParams, iteration : int):
    ...
    return gaussianModel, scene, anchors
...
if __name__ == "__main__":
    ...
    gaussianModel, scene, anchor_points = setup_gaussian_scene_and_model(
        model.extract(args), 
        args.iteration,
        )
    ...
    def build_precomputed(anchor_points: np.ndarray, projection_data: np.ndarray, eps: float, k_candidates: int = 512):
        ...
        for i in range(N):
            ...
            d = agreement_many(mask_ids[i], mask_ids[neigh])
    ...
    o3d.io.write_triangle_mesh("wyniki_inst/precomp_eps0.1_kn512_agronly_onscan1model.ply", mesh, write_vertex_colors=True)
